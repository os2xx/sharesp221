...
{
	pr_info("%s %s%s\n", ZCZCHEADER, DESCRIPTION, ": Read from and write to /proc - START");
    ...
    pr_info("%s %s /proc/%s created\n", ZCZCHEADER, DESCRIPTION, PROCFS_NAME);
    return 0;
}
...
{
    proc_remove(our_proc_file);
    pr_info("%s %s /proc/%s removed\n", ZCZCHEADER, DESCRIPTION, PROCFS_NAME);
	pr_info("%s %s%s\n", ZCZCHEADER, DESCRIPTION, ": Read from and write to /proc - STOP");
}
